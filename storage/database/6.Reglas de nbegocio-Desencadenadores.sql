...
CREATE TRIGGER des_OBSINSCRIPCION ON INSCRIPCION AFTER INSERT,UPDATE AS
    ...
    IF @OBS IS NULL
        UPDATE INSCRIPCION SET OBSERVACIONES = 'Inscripción Regular' WHERE CI=@CI AND IDCURSO=@IDCURSO AND IDNIVEL=@IDNIVEL
GO
...
CREATE TRIGGER des_ESTADOMATERIA ON MAESTROMATER AFTER INSERT AS
    ...
    SELECT @IDMATERIA = IDMATERIA, @IDCURSO = IDCURSO, @IDNIVEL = IDNIVEL FROM INSERTED 
    
    UPDATE MATERIA SET DESCRIPCION = DESCRIPCION + ' - Asignada' 
    WHERE IDMATERIA=@IDMATERIA AND IDCURSO=@IDCURSO AND IDNIVEL=@IDNIVEL
GO
...
CREATE TRIGGER des_ESTADOMATERIA1 ON MAESTROMATER AFTER DELETE AS
    ...
    SELECT @IDMATERIA = IDMATERIA, @IDCURSO = IDCURSO, @IDNIVEL = IDNIVEL FROM DELETED 
