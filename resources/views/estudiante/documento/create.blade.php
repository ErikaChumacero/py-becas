<x-student-layout>
    ...
    <div class="w-full max-w-2xl p-6 bg-white rounded shadow">
        ...
        <form method="POST" action="{{ route('estudiante.documento.store') }}" class="space-y-4" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Máx. 20 MB. Formato: PDF.</p>
                </div>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const oldReq = "{{ old('idrequisito') }}";
                async function cargarRequisitos(idpostulacion) {
                    if (!idpostulacion) {
                        selReq.innerHTML = '<option value="">Seleccione...</option>';
                        return;
                    }
                    try {
                        ...
                        data.forEach(r => {
                            ...
                            if (oldReq && oldReq == r.idrequisito) opt.selected = true;
