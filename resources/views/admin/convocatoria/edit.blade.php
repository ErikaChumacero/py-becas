<x-admin-layout>
    <h1 class="text-2xl font-semibold mb-4">Editar Convocatoria</h1>

    @error('general')
        <div class="mb-4 p-3 rounded bg-rose-100 text-rose-800">{{ $message }}</div>
    @enderror

    <form method="POST" action="{{ route('admin.convocatoria.update', $row->idconvocatoria) }}" class="space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-sm font-medium mb-1">Título</label>
            <input type="text" name="titulo" class="w-full border rounded p-2" value="{{ old('titulo', $row->titulo) }}" maxlength="100" required>
            @error('titulo')<div class="text-rose-600 text-sm mt-1">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block text-sm font-medium mb-1">Descripción</label>
            <textarea name="descripcion" class="w-full border rounded p-2" maxlength="500" required>{{ old('descripcion', $row->descripcion) }}</textarea>
            @error('descripcion')<div class="text-rose-600 text-sm mt-1">{{ $message }}</div>@enderror
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1">Fecha inicio</label>
                <input type="date" name="fechainicio" class="w-full border rounded p-2" value="{{ old('fechainicio', \Carbon\Carbon::parse($row->fechainicio)->format('Y-m-d')) }}" required>
            ...
            <div>
                ...
                <input type="date" name="fechafin" class="w-full border rounded p-2" value="{{ old('fechafin', \Carbon\Carbon::parse($row->fechafin)->format('Y-m-d')) }}" required>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="idtipobeca" class="w-full border rounded p-2" required>
                    @foreach($tipos as $t)
                        <option value="{{ $t->idtipobeca }}" @selected(old('idtipobeca', $row->idtipobeca)==$t->idtipobeca)>{{ $t->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="estado" class="w-full border rounded p-2" required>
                    <option value="1" @selected(old('estado', $row->estado)==='1')>Activa</option>
                    <option value="0" @selected(old('estado', $row->estado)==='0')>Inactiva</option>
                </select>
