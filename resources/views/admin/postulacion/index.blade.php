<x-admin-layout>
    ...
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-xl font-semibold">Postulaciones</h1>
        <a href="{{ route('admin.postulacion.create') }}" class="px-3 py-2 rounded bg-blue-600 hover:bg-blue-500 text-white">Nueva postulación</a>
    </div>
        ...
        <tbody>
            @forelse($rows as $p)
                <tr class="border-t">
                    ...
                    <td class="p-2">{{ $p->estudiante }} ({{ $p->ci }})</td>
                    ...
                    <td class="p-2">
                        @php
                            ...
                            $e = strval($p->estado);
                        @endphp
                        <span class="px-2 py-0.5 rounded text-xs {{ $classes[$e] ?? 'bg-gray-100 text-gray-800' }}">{{ $labels[$e] ?? $p->estado }}</span>
                    </td>
                    <td class="p-2 space-x-2">
                        <a href="{{ route('admin.postulacion.edit', $p->idpostulacion) }}" class="px-2 py-1 rounded bg-amber-500 hover:bg-amber-400 text-white">Editar</a>
                    </td>
                </tr>
            @empty
                <tr>
