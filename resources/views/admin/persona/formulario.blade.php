    ...
    <div class="w-full max-w-xl p-6 bg-white rounded shadow">
        ...
        <form action="{{ route('admin.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="tipoe" value="1" {{ old('tipoe') ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="codigo" id="codigo" value="{{ old('codigo') }}" class="mt-1 block w-full p-2 border border-gray-300 rounded">
                    @error('codigo')
                        <div class="text-red-600">{{ $message }}</div>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" name="nroregistro" id="nroregistro" value="{{ old('nroregistro') }}" class="mt-1 block w-full p-2 border border-gray-300 rounded">
                    @error('nroregistro')
                        <div class="text-red-600">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <a href="{{ route('admin.index') }}" class="px-3 py-2 bg-gray-200 rounded">Cancelar</a>
