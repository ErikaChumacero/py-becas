<x-admin-layout>
    ...
    <div class="w-full max-w-2xl p-6 bg-white rounded shadow">
        <h2 class="text-2xl font-bold mb-4">Nuevo Requisito</h2>
        ...
        <form method="POST" action="{{ route('admin.requisito.store') }}" class="space-y-4">
            @csrf
            <div>
                <label class="block text-gray-700">Descripción</label>
                <textarea name="descripcion" class="mt-1 block w-full p-2 border rounded" rows="3" required>{{ old('descripcion') }}</textarea>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="idtipobeca" class="mt-1 block w-full p-2 border rounded" required>
                        ...
                        @foreach($tipos as $t)
                        <option value="{{ $t->idtipobeca }}" {{ old('idtipobeca')==$t->idtipobeca ? 'selected' : '' }}>{{ $t->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
