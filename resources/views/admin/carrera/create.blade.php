<x-admin-layout>
    <h1 class="text-2xl font-semibold mb-4">Nueva Carrera</h1>

    <form method="POST" action="{{ route('admin.carrera.store') }}" class="space-y-4">
        @csrf
        <div>
            <label class="block text-sm font-medium mb-1">Plan</label>
            <input type="text" name="plancarrera" class="w-full border rounded p-2" value="{{ old('plancarrera') }}" maxlength="50" required>
            @error('plancarrera')<div class="text-rose-600 text-sm mt-1">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block text-sm font-medium mb-1">Código</label>
            <input type="text" name="codigo" class="w-full border rounded p-2" value="{{ old('codigo') }}" maxlength="20" required>
            @error('codigo')<div class="text-rose-600 text-sm mt-1">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block text-sm font-medium mb-1">Nombre</label>
            <input type="text" name="nombre" class="w-full border rounded p-2" value="{{ old('nombre') }}" maxlength="100" required>
