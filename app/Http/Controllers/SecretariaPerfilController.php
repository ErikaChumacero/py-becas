...
class SecretariaPerfilController extends Controller
{
    private function currentSecretariaCi(): ?string
    {
        $u = session('usuario');
        return $u['ci'] ?? null;
    }
    ...
    {
        $ci = $this->currentSecretariaCi();
        if (!$ci) { abort(403); }

        try {
            // Obtener información de la secretaria
            $secretaria = DB::selectOne(<<<SQL
                SELECT 
                    ci,
                    nombre,
                    apellido,
            ...
            return view('secretaria.perfil.index', compact('secretaria'));
        } catch (\Throwable $e) {
            return redirect()->route('secretaria.dashboard')
